...
def render_client_standings(metrics: ExecutiveMetrics, timeline: pd.DataFrame, gross_view: bool = False) -> None:
    ...
    # Tiers: Gold = top 5 clients by VISIT count (i.e. the top 5 rows in
    # this very directory, since directory is already visit-sorted).
    # Silver/Bronze are then judged on avg revenue/visit, each tier
    # excluding anyone already claimed by a higher one.
    gold_names = {str(item["client"]) for item in directory[:5]}
    ...
    legend_html = (
        ...
        '<div class="tier-legend-item"><span class="tier-pill tier-gold">GOLD</span><span>TOP 5 CLIENT</span></div>'
        '<div class="tier-legend-item"><span class="tier-pill tier-silver">SILVER</span><span>\uFF04'
        '98+/VISIT AVG</span></div>'
    ...
    *{{box-sizing:border-box;margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}}html,body{{width:100%;overflow:hidden;background:transparent}}body{{color:#fff}}
    ...
    .tier-legend{{display:grid;grid-template-columns:1fr 1fr;gap:4px 10px;margin-bottom:14px;max-height:1.5rem}}
    .tier-legend-item{{display:flex;align-items:center;gap:5px;font-size:7.5px;font-weight:800;letter-spacing:.3px;color:#94a3b8;text-transform:uppercase;line-height:1}}
    ...
    </style></head><body><div class="client-card-container"><div class="client-header-title">CLIENT STANDINGS</div><div class="client-overline">\u25cf FULL DIRECTORY, RANKED BY VISITS</div>{legend_html}<input id="client-search" type="search" class="search-box" placeholder="Search current client directory..."><div class="client-list">{''.join(rows)}</div></div><script>
